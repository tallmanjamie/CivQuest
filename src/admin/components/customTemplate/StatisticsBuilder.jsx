...
function StatisticModal({ statistic, existingIds, availableFields, onSave, onCancel, isEdit }) {
  ...
  const [showFilterSection, setShowFilterSection] = useState(
    statistic?.filter?.mode && statistic?.filter?.mode !== 'none'
  );
  ...
  const handleFilterModeChange = (mode) => {
    setFormData(prev => ({
      ...
      filter: {
        ...prev.filter,
        mode,
        rules: mode === 'simple' && (!prev.filter.rules || prev.filter.rules.length === 0)
  ...
  const addFilterRule = () => {
    setFormData(prev => ({
      ...
      filter: {
        ...prev.filter,
        rules: [...(prev.filter.rules || []), { field: '', operator: '=', value: '' }]
      }
    }));
  };
  ...
  const updateFilterRule = (index, field, value) => {
    setFormData(prev => {
      const rules = [...(prev.filter.rules || [])];
      rules[index] = { ...rules[index], [field]: value };
      return {
        ...prev,
        filter: { ...prev.filter, rules }
      };
    });
  };
  ...
  const removeFilterRule = (index) => {
    setFormData(prev => ({
      ...
      filter: {
        ...prev.filter,
        rules: prev.filter.rules.filter((_, i) => i !== index)
      }
    }));
  };
  ...
  const toggleFilterLogic = () => {
    setFormData(prev => ({
      ...
      filter: {
        ...prev.filter,
        logic: prev.filter.logic === 'AND' ? 'OR' : 'AND'
      }
    }));
  };

  const handleAdvancedWhereChange = (value) => {
    setFormData(prev => ({
      ...prev,
      filter: { ...prev.filter, advancedWhere: value }
    }));
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="border-t pt-4 mt-4">
            ...
            >
              {showFilterSection ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
              <Filter className="w-4 h-4" />
              <span>Filter Records (Optional)</span>
              {formData.filter.mode !== 'none' && (
                <span className="ml-auto text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">
                  {formData.filter.mode === 'simple'
                    ? `${formData.filter.rules?.filter(r => r.field && r.value).length || 0} rule(s)`
                    : 'Advanced'}
                </span>
              )}
            </button>
            ...
            {showFilterSection && (
              <div className="mt-3 space-y-3">
                <p className="text-xs text-slate-500">
                  Filter which records are included in this statistic calculation.
                </p>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs rounded border transition-colors ${
                        formData.filter.mode === mode.id
                          ? 'bg-[#004E7C] text-white border-[#004E7C]'
                ...
                {formData.filter.mode === 'simple' && (
                  <div className="space-y-2">
                    {/* Logic Toggle */}
                    {formData.filter.rules?.length > 1 && (
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {formData.filter.logic === 'AND' ? 'ALL rules' : 'ANY rule'}
                        </button>
                      </div>
                    )}

                    {/* Rules */}
                    {(formData.filter.rules || []).map((rule, index) => (
                    ...
                    <button
                      ...
                      className="text-xs text-[#004E7C] hover:text-[#003d61] flex items-center gap-1"
                    >
                ...
                {formData.filter.mode === 'advanced' && (
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">
                      WHERE Clause
                    </label>
                    <textarea
                      value={formData.filter.advancedWhere}
                      onChange={(e) => handleAdvancedWhereChange(e.target.value)}
                      placeholder="e.g., STATUS = 'Active' AND VALUE > 1000"
                      rows={3}
                      className="w-full px-3 py-2 border border-slate-200 rounded text-xs font-mono"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                      Enter a SQL WHERE clause to filter records for this statistic.
                    </p>
                  </div>
                )}
              </div>
          ...
          {statistics.map((stat, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {stat.filter && stat.filter.mode !== 'none' && (
                  <div className="flex items-center gap-1 mt-1">
                    <Filter className="w-3 h-3 text-blue-500" />
                    <span className="text-[10px] text-blue-600">
                      {stat.filter.mode === 'simple'
                        ? `Filtered (${stat.filter.rules?.filter(r => r.field && r.value).length || 0} rule${(stat.filter.rules?.filter(r => r.field && r.value).length || 0) !== 1 ? 's' : ''})`
                        : 'Filtered (custom)'}
                    </span>
                  </div>
                )}
