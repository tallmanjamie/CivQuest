...
/**
 * Generate PDF content for feature export
 */
function generatePDFContent(title, displayFields, config) {
  const appName = config?.messages?.welcomeTitle || config?.ui?.title || 'Atlas';
  const logo = config?.ui?.logoLeft || '';
  const themeColor = config?.ui?.themeColor || 'sky';

  // Theme color mapping for PDF
  const themeColors = {
    sky: '#0284c7',
    blue: '#2563eb',
    indigo: '#4f46e5',
  ...
  const primaryColor = themeColors[themeColor] || themeColors.sky;
  ...
  const rows = displayFields.map(field => `
    <tr>
      <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-weight: 500; color: #475569; text-transform: uppercase; font-size: 11px; width: 40%; vertical-align: top;">${field.label}</td>
      <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; color: #1e293b;">${formatPDFValue(field.value)}</td>
    </tr>
  `).join('');
    ...
    <body>
      <div class="header">
        ...
        <div class="header-text">
          <h1>${title}</h1>
          <p>Generated from ${appName} on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}</p>
        </div>
      </div>
      <div class="content">
        <table>
      ...
      <div class="footer">
        <p>This document was generated by ${appName}. Data shown is for informational purposes only.</p>
      </div>
    </body>
    </html>
  `;
...
Remember to respond with ONLY a valid JSON object, no additional text or markdown.`;
  ...
  /**
   * Export feature details to PDF
   */
  const exportFeatureToPDF = useCallback((feature) => {
    if (!feature?.attributes) return;

    const tableColumns = activeMap?.tableColumns || [];
    const attrs = feature.attributes;

    // Build display fields from tableColumns or auto-generate
    let displayFields;
    if (tableColumns.length > 0) {
      displayFields = tableColumns
        .filter(col => attrs[col.field] != null && attrs[col.field] !== '')
        .map(col => ({
          label: col.headerName || col.field.replace(/_/g, ' '),
    ...
    const title = attrs[addressField] || attrs.PROPERTYADDRESS || attrs.ADDRESS || attrs.PARCELID || 'Property Details';

    // Generate PDF HTML content
    const pdfContent = generatePDFContent(title, displayFields, config);
  ...
  }, [activeMap?.tableColumns, getFieldNames, config]);
      ...
      >
        ...
        {messages.map((msg) => (
          <MessageBubble
            ...
            onExportPDF={exportFeatureToPDF}
 ...
 */
function MessageBubble({ message, botAvatar, colors, onViewMap, onViewTable, onExportPDF, tableColumns, enabledModes }) {
      ...
      <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm border border-slate-200 max-w-[80%]">
        ...
        {message.showDetails && message.feature && (
          <div className="mt-3 pt-3 border-t border-slate-100">
            ...
            {/* Action buttons for single result */}
 ...
 * Uses tableColumns configuration to display fields with proper header names
 */
function FeatureDetails({ feature, colors, tableColumns }) {
  const attrs = feature.attributes || {};

  // Build display fields from tableColumns configuration if available
  const displayFields = React.useMemo(() => {
    if (tableColumns && tableColumns.length > 0) {
      // Use tableColumns configuration - show all configured columns
      return tableColumns
        .filter(col => {
          const value = attrs[col.field];
    ...
    // Fallback: auto-generate from attributes (excluding internal fields)
    ...
    <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
      {displayFields.map(({ key, label, value }) => (
        <div key={key}>
          ...
          <dd className="text-slate-800">{formatValue(value)}</dd>
        </div>
      ))}
    </dl>
  );
